  ...
  return (
    <div className="min-h-screen pb-20" style={{ background: themeColors.backgroundGradient }}>
      ...
      <main className="px-4 py-8">
        {/* Order ID Banner */}
        <div className="bg-white/80 rounded-2xl px-5 py-3 mb-6 flex items-center justify-between shadow-sm border border-white/60">
          <div>
            <p className="text-xs font-bold text-slate-400 uppercase">Delivery Order</p>
            <p className="font-black text-slate-900">#{order.orderId}</p>
          </div>
          <div className="text-right">
            <p className="text-xs font-bold text-slate-400 uppercase">Amount</p>
            <p className="font-black text-slate-900">₹{order.financialBreakdown?.totalAmount || 0}</p>
          </div>
        </div>
        ...
        <div
          ...
          style={{ border: '1px solid rgba(255,255,255,0.3)', boxShadow: '0 20px 40px rgba(0,0,0,0.1)' }}
        >
              ...
              return (
                <div key={stage.id} className="relative pb-10 last:pb-0">
                  ...
                  <div className="flex items-start gap-6 relative">
                    {isCurrent && <div className="absolute left-0 top-0 w-12 h-12 rounded-full animate-ping bg-blue-400/20" />}

                    <div
                      className="relative z-10 w-12 h-12 rounded-2xl flex items-center justify-center flex-shrink-0 transition-all duration-500 ease-out text-xl"
                      style={{
                    ...
                    >
                      {isCompleted ? <FiCheck className="w-6 h-6 text-white" /> : stage.icon}
                    </div>
