...
const CashLimitModal = () => {
  ...
  useEffect(() => {
    ...
    const isPublicRoute = location.pathname === '/vendor/login' || 
                         ...
                         location.pathname === '/vendor/wallet/settle';
  ...
  const isPending = walletData?.pendingSettlements > 0;
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm p-6 text-center animate-bounce-in relative">
        ...
        {isPending ? (
          <>
            ...
            <h2 className="text-xl font-normal text-gray-900 mb-2">Settlement Pending</h2>

            <p className="text-gray-600 mb-6 text-sm">
              Your payment reference has been submitted. The admin is currently verifying your settlement request. Your services will resume as soon as the payment is approved.
            </p>

            <button
              onClick={() => {
                setShow(false);
                navigate('/vendor/wallet/settlements');
              }}
              className="w-full py-3 bg-amber-600 text-white font-normal rounded-xl shadow-lg shadow-amber-200 hover:bg-amber-700 transition-all active:scale-95"
            >
