  ...
  return (
    <div className="h-screen flex flex-col relative bg-white overflow-hidden">
      ...
      <div className="absolute top-4 left-4 right-4 z-20 flex justify-between items-start pointer-events-none">
        <button
          onClick={() => navigate(`/vendor/booking/${id}`, { replace: true })}
          className="pointer-events-auto bg-white/90 backdrop-blur-md p-3 rounded-full shadow-lg text-gray-700 hover:bg-white transition-all active:scale-95 cursor-pointer"
        >
      ...
      {/* Modern Bottom Card */}
      <div className={`absolute bottom-0 left-0 right-0 bg-white rounded-t-2xl shadow-[0_-8px_30px_rgba(0,0,0,0.12)] z-20 p-4 pb-20 md:pb-6 transition-transform duration-300 ${isFullScreen ? 'translate-y-full' : ''}`}>
        ...
        <div className="flex gap-2.5">
          ...
          {(booking?.userId?.phone || booking?.customerPhone) && (
            <a href={`tel:${booking.userId?.phone || booking.customerPhone}`} className="flex-1 bg-teal-600 hover:bg-teal-700 text-white font-semibold text-sm py-2.5 rounded-xl flex items-center justify-center gap-1.5 shadow-md shadow-teal-600/20 transition-all active:scale-95 whitespace-nowrap">
