...
const ProductDetailsPage = () => {
  ...
  useEffect(() => {
    const fetchDetails = async () => {
      try {
        setLoading(true);
        // Note: Using the catalog service to get single service details
        const res = await publicCatalogService.getServiceDetails(id);
        if (res.success) {
          setProduct(res.service);
        }
  ...
  const handleAddToCart = async () => {
    try {
      ...
      const cartItemData = {
        serviceId: product.id || product._id,
        title: product.title,
  ...
  const gallery = product.images?.length > 0 
    ? product.images.map(img => toAssetUrl(img))
    : [toAssetUrl(product.iconUrl || product.icon || '')];
      ...
      <main className="max-w-[1400px] mx-auto px-6 py-12">
        ...
        <div className="flex items-center gap-2 mb-8">
          ...
          <div className="flex items-center gap-2 text-[11px] font-black uppercase tracking-widest text-gray-400">
            ...
            <span className="text-gray-900">{product.categoryTitle || 'Service'}</span>
          </div>
        </div>
          ...
          <div className="lg:col-span-7 space-y-6">
            <div className="relative aspect-[4/3] bg-white rounded-[2.5rem] overflow-hidden border border-gray-100 shadow-sm">
              ...
              <div className="absolute top-8 left-8 flex gap-3">
                <div className="px-4 py-2 bg-emerald-500 text-white text-[10px] font-black uppercase tracking-widest rounded-xl shadow-lg shadow-emerald-500/30">
                  Best Seller
                </div>
                {product.vendorName && (
          ...
          <div className="lg:col-span-5 flex flex-col">
            <div className="bg-white rounded-[2.5rem] p-10 border border-gray-100 shadow-sm flex-1">
              ...
              <p className="text-gray-500 font-medium text-lg mb-8 leading-relaxed">
                {product.description || 'Premium service offering tailored to your specific requirements.'}
              </p>

              <div className="flex items-baseline gap-4 mb-10">
                <span className="text-5xl font-black text-gray-900">₹{product.basePrice}</span>
                <span className="text-xl text-gray-400 line-through font-bold">₹{product.basePrice + 500}</span>
                <div className="px-3 py-1 bg-emerald-100 text-emerald-600 text-[10px] font-black uppercase tracking-widest rounded-lg">
                  Save ₹500
                </div>
              </div>
        ...
        <div className="mt-12 grid grid-cols-1 lg:grid-cols-12 gap-12">
            ...
            <div className="bg-white rounded-[2.5rem] p-12 border border-gray-100 shadow-sm">
              ...
              <div className="prose prose-blue max-w-none">
                <p className="text-gray-600 leading-loose text-lg whitespace-pre-wrap">
                  {product.detailedDescription || product.description || 'No detailed description provided.'}
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="bg-white rounded-[2.5rem] p-10 border border-gray-100 shadow-sm">
                ...
                <ul className="space-y-4">
                  {(product.features?.length > 0 ? product.features : ['Professional Service', 'Expert Handling', 'Quality Materials']).map((f, idx) => (
              ...
              <div className="bg-white rounded-[2.5rem] p-10 border border-gray-100 shadow-sm">
                ...
                <ul className="space-y-4">
                  {(product.benefits?.length > 0 ? product.benefits : ['Guaranteed Satisfaction', 'Competitive Pricing', 'Verified Partners']).map((b, idx) => (
                    <li key={idx} className="flex items-start gap-4">
          ...
          <div className="lg:col-span-4">
            <div className="sticky top-32 space-y-6">
              <div className="bg-[#00246b] rounded-[2.5rem] p-10 text-white overflow-hidden relative shadow-2xl">
                ...
                <button 
                  onClick={handleAddToCart}
                  className="w-full bg-white text-[#00246b] py-5 rounded-2xl font-black uppercase tracking-widest text-[10px] shadow-xl hover:bg-gray-50 transition-all active:scale-95"
                >
                  Quick Checkout
                </button>
              </div>
